...
let API_KEY = '962a8a054679493803f9f67b32639591';

/* -------- API Key Management -------- */
function loadApiKey() {
  const stored = localStorage.getItem('skypulse_api_key');
  if (stored) {
    API_KEY = stored;
  } else {
...
function renderAll(data) {
  ...
  document.getElementById('uvIndex').textContent    = data.uv ? uvLabel(Math.round(data.uv)) : 'N/A';
...
async function getLocation() {
  ...
  navigator.geolocation.getCurrentPosition(async pos => {
    ...
    try {
      ...
      const [cur, fc, aqiData] = await Promise.all([curRes.json(), fcRes.json(), aqiRes.json()]);
      let uv = 0;
      try {
